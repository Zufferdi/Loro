/* ============================================================
   cross_2024_2025.js — viz cross-année sur 5 ans (2021-2025).
   Side-by-side bars montrant les top bénéficiaires année par année.
   Révèle one-shots vs récurrents.
   Note : le nom de fichier est figé pour compat URL/HTML, mais la
   viz consomme désormais cross_2021_2025_top.json (100 entries).
   ============================================================ */
(function() {
  function init() {
  ...
  function render(container) {
    ...
    fetch('data/cross_2021_2025_top.json')
      .then(r => r.json())
  ...
  function doRender(container, data) {
    ...
    // Quick stats — schéma 5 ans : champs montant_YYYY_CHF, nom, canton, nb_years_active
    const n = list.length;
    const stable5y = list.filter(b => b.nb_years_active === 5).length;
    // One-shots = présent une seule année avec montant ≥ 500k
    const isOneShot = b => {
      const presentYears = YEARS.filter(y => (b[`montant_${y}_CHF`] || 0) > 0);
      if (presentYears.length !== 1) return false;
      return (b[`montant_${presentYears[0]}_CHF`] || 0) >= 500000;
    };
    const oneShots = list.filter(isOneShot).length;
    ...
    stats.innerHTML = `
      ...
      <div class="cross-stat">
        ...
        <div class="cross-stat-lbl">one-shots (1 année, ≥&nbsp;500&nbsp;k)</div>
      </div>
    `;
    ...
    ctrls.innerHTML = `
      ...
      <button class="cross-filter" data-filter="rising">📈 En hausse 21-25</button>
      ...
      <button class="cross-filter" data-filter="5year">5 années complètes</button>
    `;
    ...
    function classify(b) {
      if (isOneShot(b)) return 'oneshot';
      const a21 = b.montant_2021_CHF || 0;
      const a25 = b.montant_2025_CHF || 0;
      // Évolution 5 ans
      const dp21_25 = a21 > 0 ? ((a25 - a21) / a21 * 100) : 0;
    ...
    function renderList(filter) {
      ...
      filtered.forEach(b => {
        ...
        const pctStr = (cls === 'oneshot')
          ? 'one-shot'
          : (a21 === 0 ? 'nouveau' : ((dp21_25 >= 0 ? '+' : '') + dp21_25 + '% (21-25)'));
        ...
        const displayName = b.nom || b.nom_2025 || b.nom_2024 || '(sans nom)';
        row.innerHTML = `
          <div class="cross-row-head">
            <span class="treemap-canton ${cantonClass(b.canton)}">${escapeHtml(b.canton || '?')}</span>
            <span class="cross-name" title="${escapeHtml(displayName)}">${escapeHtml(displayName)}</span>
