...
EXTRA_PATTERNS = {
    ...
    'social': [
        ...
        # EMS & maisons pour personnes âgées — fusionnés en 'Personnes âgées'
        (r"\bHome\s+(?:Saint\s+Pierre|St[\s-]Pierre)\b|\bPetershöfli\b", 'EMS et personnes âgées'),
        (r"\bLa\s+Rozavère\b|\bEMS\s+\w", 'EMS et personnes âgées'),
        (r"\bFond\.\s+de\s+l['\u2019]?Orme\b", 'EMS et personnes âgées'),
        (r"\bStiftung\s+Martinsheim\b|\bMartinsheim\b", 'EMS et personnes âgées'),
        (r"\bAltersheim\b|\bAlters-?\s+und\s+Pflegeheim\b|\bAlters[-\s]?wohnheim\b", 'EMS et personnes âgées'),
        (r"\bSt\.\s+Antonius\s+Alters\b", 'EMS et personnes âgées'),
        (r"\bFond\.\s+Chez\s+Paou\b", 'EMS et personnes âgées'),
        (r"\bRésidence\s+(?:de\s+|du\s+)?(?:Bonne[-\s]Espérance|La\s+Courtine|Le\s+Carillon|Les\s+Glariers|Saphir|Saint-?Joseph|Beauregard|Belmont)\b", 'EMS et personnes âgées'),
        (r"\bAlzheimer\b", 'EMS et personnes âgées'),
        (r"\bSAPHIR\b(?!\s+(?:bijoux|joaillerie))", 'EMS et personnes âgées'),  # Fond. SAPHIR = EMS Yverdon
        (r"\bFond\.\s+(?:du\s+)?Saphir\b", 'EMS et personnes âgées'),
        (r"\bAccueil\s+de\s+jour\s+aînés?\b", 'EMS et personnes âgées'),
        (r"\bFond\.\s+pour\s+(?:la\s+)?vieillesse\b", 'EMS et personnes âgées'),
        (r"\bMaison\s+(?:du\s+|de\s+la\s+)?(?:Mont|Vieux|Pré|Tilleul)\b.*EMS\b", 'EMS et personnes âgées'),
